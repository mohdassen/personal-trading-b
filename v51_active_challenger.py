"""V5.1 research-only active challenger. Never trades or modifies frozen V4.9."""
...
def qualify(rows):
 ...
 for r in rows:
  ...
  score+=10 if rs else 0; score+=5 if entry_quality else 0; score+=5 if r.get('leadership_health_ok') else 0
  x['score']=int(round(min(100,score))); out.append(x)
 return out
...
def main():
 ...
 prices=pm._load_complete_prices(list(dict.fromkeys(['SPY']+list(universe)))); sf=em._spy_regime(prices['SPY']); raw=[]
 for s in universe: raw.extend(em._signal_rows(s,prices[s],sf))
 rows=qualify(am._enrich_leadership(raw,prices)); original=pm.SCORE_GRID
 try:
  pm.SCORE_GRID=SCORE_GRID; validation=pm._validate(rows,prices,cost)
