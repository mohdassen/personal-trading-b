...
OUT=Path("data/ibs_lower_band_round11.json")
START="2005-01-01"
BASE_BPS=10.0
STRESS_BPS=30.0
SYMBOLS=["IVV","IWB","IWF","IWD","RSP","VUG","VTV","VO"]
...
def main():
    ...
    out={"strategy":"IBS_LOWER_BAND_US_ETF_REPLICATION","rules":"close below 10d-high minus 2.5x 25d avg range, IBS<0.30, close>SMA200; next-open entry; exit next open after close>prior high or close<SMA200","base_bps":BASE_BPS,"stress_bps":STRESS_BPS,"results":results,"period_gates":gates,"status":"ROUND11_REPLICATION_CANDIDATE" if passed else "REJECT","limitations":["daily adjusted Yahoo bars are research-grade","symbols are a new US equity ETF replication set not used in Round 10; rule and gate are unchanged","candidate would require portfolio overlap/risk simulation and forward validation"]}
