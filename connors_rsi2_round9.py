...
OUT=Path("data/connors_rsi2_round9.json")
START="2005-01-01"
BASE_BPS=10.0
STRESS_BPS=30.0
...
def trades(df,bps):
    ...
    while i<n-2:
        row=x.iloc[i]
        if not (row.Close>row.sma200 and row.rsi2<5):
            i+=1;continue
...
def main():
    ...
    out={"strategy":"CONNORS_RSI2_LT5_INDEX_ETF","rules":"long when close>SMA200 and RSI2<5; next-open entry; exit next open after close>SMA5; no stop; non-overlapping per ETF",
         "base_bps":BASE_BPS,"stress_bps":STRESS_BPS,"results":results,"period_gates":gates,"status":"ROUND9_CANDIDATE" if passed else "REJECT",
