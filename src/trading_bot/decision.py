...
def _trade_instruction(setup, signal, score):
    ...
    if signal == "STRONG_BUY":
        ...
        if low <= price <= high:return "BUY_NOW", f"Buy only inside ${low:.2f}-${high:.2f}"
        return "DO_NOT_CHASE", f"Price is above entry zone; do not chase above ${high:.2f}"
    if signal == "BUY":
        if low <= price <= high:return "BUY_NOW", f"Buy inside ${low:.2f}-${high:.2f}"
        if price < low:return "WAIT_FOR_ENTRY", f"Wait for pullback/entry zone ${low:.2f}-${high:.2f}"
...
def _confirmation_gate(setup, regime, signal):
    """Conservative second opinion: a high score alone is not enough to trigger a buy."""
    ...
    if strategy=="DAY":
        checks=[
            (setup.vol_ratio>=1.2,"حجم التداول يدعم الحركة"),
            (setup.momentum>0,"الزخم قصير المدى إيجابي"),
            (48<=setup.rsi<=70,"الحركة ليست ضعيفة أو مفرطة"),
        ]
    else:
        checks=[
            (setup.vol_ratio>=1.05,"حجم التداول مقبول"),
            (setup.momentum>0,"اتجاه الأيام الأخيرة إيجابي"),
            (47<=setup.rsi<=68,"قوة السهم ضمن نطاق صحي"),
        ]
    passed=sum(1 for ok,_ in checks if ok)
    required=3 if signal=="STRONG_BUY" else 2
    notes=[text for ok,text in checks if ok]
    if passed<required:
        return False, f"only {passed}/{len(checks)} confirmations passed", notes
...
def _simple_view(signal, action, score, event_level, risk_pct, low, high, stop, target1):
    if action=="BUY_NOW":
        decision_ar="اشترِ الآن"; decision_en="BUY NOW"; next_step=f"ادخل فقط بين ${low:.2f} و ${high:.2f}. ضع وقف الخسارة عند ${stop:.2f}. الهدف الأول ${target1:.2f}."
    elif action=="WAIT_FOR_ENTRY":
        decision_ar="انتظر السعر المناسب"; decision_en="WAIT"; next_step=f"لا تشترِ الآن. انتظر وصول السعر إلى منطقة ${low:.2f}–${high:.2f}."
    elif action=="DO_NOT_CHASE":
        decision_ar="لا تطارد السعر"; decision_en="DO NOT BUY NOW"; next_step=f"السعر ارتفع فوق منطقة الدخول. انتظر رجوعه إلى ${low:.2f}–${high:.2f}."
    elif action=="WATCH":
        decision_ar="راقب فقط"; decision_en="WATCH"; next_step="الفرصة لم تكتمل بعد. لا تدخل الصفقة الآن."
    else:
        decision_ar="لا تدخل"; decision_en="NO TRADE"; next_step="الشروط غير مكتملة حاليًا. احتفظ بالكاش وانتظر فرصة أوضح."
...
def finalize(setup, regime, settings, portfolio, equity):
    ...
    sizing=size_position(setup.price,setup.stop_loss,equity,float(settings.get("risk_per_trade_pct",.5)),float(settings.get("max_position_pct",15)),cash)
    ...
    if signal in ("BUY","STRONG_BUY") and sizing["shares"]<=0:
        signal="BLOCKED"; action="NO_TRADE"; instruction="Position sizing returned zero shares; capital/risk settings block the trade"
    ...
    return {**setup.to_dict(),"score":score,"signal":signal,"action":action,"instruction":instruction,**simple,"market_regime":regime.label,"event_risk":event.level,"event_notes":event.notes,"market_notes":regime.notes,"confirmation_passed":confirmed,"confirmation_reason":confirmation_reason,"confirmation_notes":confirmation_notes,"suggested_shares":sizing["shares"],"suggested_value":sizing["position_value"],"risk_dollars":sizing["risk_dollars"],"risk_pct_equity":sizing["risk_pct_equity"],"portfolio_gate":gate_reason}
