...
OUT=Path("data/ibs_exact_round12.json")
START="2005-01-01"
BASE_BPS=10.0
STRESS_BPS=30.0
SYMBOLS=["SPY","QQQ","DIA","IWM","EFA","EEM","MDY","VTI","IVV","IWB","IWF","IWD","RSP","VUG","VTV","VO"]
...
def main():
    ...
    passed=all(gates[p]["baseline"]["eligible_symbols"]>=14 and gates[p]["baseline"]["passing_symbols"]>=12 and gates[p]["stress"]["passing_symbols"]>=10 for p in periods)
    out={"strategy":"IBS_LOWER_BAND_EXACT_PUBLISHED","rules":"close below 10d-high minus 2.5x 25d avg range, IBS<0.30, ; next-open entry; exit next open after close>prior high or close<SMA300","base_bps":BASE_BPS,"stress_bps":STRESS_BPS,"results":results,"period_gates":gates,"status":"ROUND12_CANDIDATE" if passed else "REJECT","limitations":["daily adjusted Yahoo bars are research-grade","corrected exact published logic: no entry trend filter and 300-day SMA exit; fills remain conservative next-open","candidate would require portfolio overlap/risk simulation and forward validation"]}
