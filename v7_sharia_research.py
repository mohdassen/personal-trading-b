...
def _business_status(info, sector_hint=""):
    text = " ".join(str(x or "") for x in (sector_hint, info.get("sector"), info.get("industry"), info.get("longBusinessSummary"))).lower()
    hits = sorted({term for term in PROHIBITED_TERMS if term in text})
    return ("FAIL", hits) if hits else ("PASS", [])


def _market_cap(t, info):
    market_cap = None
    try:
        fi = t.fast_info
        if hasattr(fi, "get"):
            market_cap = _f(fi.get("market_cap")) or _f(fi.get("marketCap"))
        else:
            try: market_cap = _f(fi["market_cap"])
            except Exception: pass
    except Exception: pass
    if not market_cap:
    ...
    if not market_cap and shares:
        try:
            ...
            if hist is not None and not hist.empty:
                market_cap = shares * _f(hist["Close"].dropna().iloc[-1])
        except Exception: pass
    return market_cap
...
def research_screen(symbol, sector_hint=""):
    ...
    out = {"method":"AAOIFI_SS21_STYLE_RESEARCH_PARTIAL","certified":False,"symbol":key[0],"status":"REVIEW_REQUIRED","reasons":[],"thresholds":{"debt_to_market_cap":DEBT_LIMIT,"cash_proxy_to_market_cap":CASH_PROXY_LIMIT,"impure_income_to_revenue":IMPURE_INCOME_LIMIT}}
    try:
        t = yf.Ticker(key[0])
        try: info = t.info or {}
        ...
        if business == "FAIL":
            out["status"]="FAIL"; out["reasons"].append("PROHIBITED_OR_REVIEW_SENSITIVE_CORE_ACTIVITY"); CACHE[key]=out; return out

        market_cap = _market_cap(t, info)
        ...
        cash_proxy = _line(bs, ("Cash Cash Equivalents And Short Term Investments","Cash And Cash Equivalents","Cash Financial"))
        ...
        out["financials"] = {"market_cap":market_cap,"interest_bearing_debt_proxy":debt,"cash_interest_deposit_proxy":cash_proxy,"debt_to_market_cap":round(debt_ratio,4) if debt_ratio is not None else None,"cash_proxy_to_market_cap":round(cash_ratio,4) if cash_ratio is not None else None}
        ...
        if out["status"] == "FAIL": CACHE[key]=out; return out
        if debt_ratio is None or cash_ratio is None:
            out["reasons"].append("MISSING_CRITICAL_FREE_FINANCIAL_DATA"); CACHE[key]=out; return out
        ...
        out["financials"].update({"interest_income_proxy":interest_income,"total_revenue":revenue,"impure_income_proxy_ratio":round(impure_ratio,4) if impure_ratio is not None else None})
        if impure_ratio is not None and impure_ratio >= IMPURE_INCOME_LIMIT:
            ...
            if impure_ratio is None: out["reasons"].append("IMPURE_INCOME_NOT_FULLY_DISCLOSED_IN_FREE_DATA")
...
def install(engine):
    ...
    def sharia_screened_picks(rows, n=3):
        shortlist = original_pick(rows, max(n * 2, n)); selected=[]
        for candidate in shortlist:
            screen = research_screen(candidate.get("symbol"), candidate.get("sector", "")); candidate["sharia_research"] = screen
            if screen.get("status") != "RESEARCH_PASS_PARTIAL": continue
